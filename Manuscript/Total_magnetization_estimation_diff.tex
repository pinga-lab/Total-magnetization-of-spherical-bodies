...
%DIF DEL Total_magnetization_estimation.tex           Tue Dec  9 21:46:14 2014
%DIF ADD Total_magnetization_estimation_revised.tex   Thu Mar 19 11:43:31 2015
...
\begin{abstract}
  ...
  inverse problem that }\DIFaddend can be applied to
...
\subsubsection{\DIFadd{Significantly overlapping anomalies}}
...
calculate the uncertainties using the propagation of covariance
(Eqs.~\ref{eq:cov-h-hat}--\ref{eq:H-h-tilde}) and assuming that the standard
deviation of the errors is equal to the true one ($\sigma = 5$}\,\DIFadd{nT). The
...
%DIF > Even in the case of the prism
%DIF > that violates the premise assumed by our method that the bodies can be
%DIF > approximated by spheres, the estimates are very close to the true ones with
%DIF > a small uncertainties. We can attribute this good performance of our method
%DIF > to three factors: (i) the absence of interfering signals produced, for
%DIF > example, by multiple magnetic sources, (ii) the simulated prism is a cube
%DIF > that seems a sphere-like body; and (iii) the use of the correct locations of
%DIF > the centres of the simulated bodies. The following tests will show how these
%DIF > factors affect the results obtained by using the least-squares and robust
%DIF > estimates.
%DIF > 
%DIF > We repeated the numerical test presented in the previous section (Sect.~3.1),
%DIF > but using the contaminated total-field anomaly shown in
%DIF > Fig.~\ref{fig:synt1-data}b. The results are shown in
%DIF > Table~\ref{tab:synt1-with-inter-anomaly}. As we can see, the results obtained
%DIF > with the least-squares estimate are very affected by the interfering
%DIF > anomalies if compared with that ones obtained by using the robust estimate.
%DIF > By comparing the true values of inclination $I$ and declination $D$ with that
%DIF > ones obtained with the least-squares estimate, the differences reach
%DIF > approximately $14${\degree} in declination and $9${\degree} in inclination.
%DIF > On the other hand, the differences between the results obtained with the
%DIF > robust estimate and the true values reach only $2${\degree} in declination
%DIF > and $3${\degree} in inclination. These results suggest that the least-squares
%DIF > estimate is more sensitive to interfering anomalies than the robust estimate.
...
$\hat{D}$ &$-10.07141$ &\DIFdelbeginFL \DIFdelFL{$-40.63733$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{D}$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00000$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL &\DIFaddbeginFL \DIFaddFL{$-40.63733$ }&\DIFaddFL{(}\DIFaddendFL $0.00113$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
...
$I$ &$-20.00000$ & \DIFaddbeginFL \DIFaddFL{-- }&\DIFaddendFL $30.00000$ \DIFaddbeginFL & \DIFaddFL{-- }\DIFaddendFL \\
$\hat{I}$ &$-19.99437$ &\DIFdelbeginFL \DIFdelFL{$31.04075$  }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{I}$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00000$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL &\DIFaddbeginFL \DIFaddFL{$31.04075$ }&\DIFaddFL{(}\DIFaddendFL $0.00068$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
...
\middlehline
$D$ &$-10.00000$ & \DIFaddbeginFL \DIFaddFL{-- }&\DIFaddendFL $-40.00000$ \DIFaddbeginFL & \DIFaddFL{-- }\DIFaddendFL \\
$\hat{D}$ &$-4.28547$ &\DIFdelbeginFL \DIFdelFL{$-23.63607$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{D}$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00000$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL &\DIFaddbeginFL \DIFaddFL{$-23.63607$  }&\DIFaddFL{(}\DIFaddendFL $0.00130$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
...
$I$ &$-20.00000$ & \DIFaddbeginFL \DIFaddFL{-- }&\DIFaddendFL $30.00000$ \DIFaddbeginFL & \DIFaddFL{-- }\DIFaddendFL \\
$\hat{I}$ &$-25.11757$ &\DIFdelbeginFL \DIFdelFL{$39.08012$  }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{I}$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00000$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL &\DIFaddbeginFL \DIFaddFL{$39.08012$  }&\DIFaddFL{(}\DIFaddendFL $0.00064$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
...
%DIFDELCMD < \includegraphics[width=70mm]{Figures/npgd-2014-0069-f07}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Robustness against errors in the centre location. The blue
...
$z$~axis. Along each axis, the magnetization direction was
  ...
  a~range of 2000}%DIFDELCMD < \,\unit{m} %%%
\DIFdelFL{on a~line passing through the right
  ...
  declination (or inclination) $\pm 5$}%DIFDELCMD < {\degree}%%%
\DIFdelFL{.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:robust-center-results}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
%%%
\DIFdelend %DIF > \begin{figure}[t]
%DIF > \includegraphics[width=70mm]{Figures/npgd-2014-0069-f07}
%DIF > \caption{Robustness against errors in the centre location. The blue
%DIF >   and red dots represent, respectively, the magnetization direction of
%DIF >   a~simulated spherical body obtained with the least-squares
%DIF >   $\hat{\vec{h}}$ (Eq.~\ref{eq:h_hat}) and robust $\tilde{\vec{h}}$
%DIF >   (Eqs.~\ref{eq:iteration-L1} and \ref{eq:ri}) estimates. The
%DIF >   estimated declinations and inclinations were obtained by presuming
%DIF >     different positions for the centre of the source along the $x$, $y$ and
%DIF > $z$~axis. Along each axis, the magnetization direction was
%DIF >   estimated by considering $21$ different centres regularly spaced in
%DIF >   a~range of 2000\,\unit{m} on a~line passing through the right
%DIF >   coordinates of the centre of the simulated spherical body (vertical
%DIF >   dashed lines). The continuous black lines represent the true
%DIF >   declinations (or inclinations). The dashed lines represent the true
%DIF >   declination (or inclination) $\pm 5${\degree}.}
%DIF > \label{fig:robust-center-results}
%DIF > \end{figure}
...
\DIFdelFL{s Alkaline Province
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Significantly overlapping anomalies. Synthetic prisms
...
\DIFaddbeginFL \caption{\DIFaddFL{Significantly overlapping anomalies. Noise-corrupted 
...
\caption{\DIFaddFL{Igneous intrusion. Vertical profile showing the 
...
on a plane with constant vertical coordinate equal to -80}\,\unit{m}\DIFaddFL{. }}
...
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=70mm]{Figures/npgd-2014-0069-f15}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=120mm]{Figures/npgd-2014-0069-f16}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=80mm]{Figures/npgd-2014-0069-f17}
\DIFaddendFL \caption{Application to field data on the Goi\'{a}s Alkaline Province
