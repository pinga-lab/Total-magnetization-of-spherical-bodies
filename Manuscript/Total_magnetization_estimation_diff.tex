...
%DIF DEL Total_magnetization_estimation.tex           Fri Mar 20 12:53:42 2015
%DIF ADD Total_magnetization_estimation_revised.tex   Fri Mar 20 20:02:10 2015
...
\begin{abstract}
  ...
  spherical shape and known centres. Our method \DIFaddbegin \DIFadd{is an overdetermined
  inverse problem that }\DIFaddend can be applied to interpret multiple sources 
  with different \DIFaddbegin \DIFadd{but homogeneous }\DIFaddend magnetization
  ...
  \DIFdelbegin \DIFdel{as the 
  }\DIFdelend ones in the alkaline complex of Montes Claros de Goi\DIFdelbegin \DIFdel{á}\DIFdelend \DIFaddbegin \'{a}\DIFaddend s, strongly 
...
\citep{telford-etal1990,blakely1996}. \DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Due to the great importance of the }\DIFdelend \DIFaddbegin \DIFadd{Several methods for determining the }\DIFaddend magnetization direction of
the sources \DIFaddbegin \DIFadd{have been developed due to its great importance }\DIFaddend in
interpreting total-field anomalies\DIFdelbegin \DIFdel{, several methods for 
determining the magnetization direction of the sourceshave been developed. These methods can be divided }\DIFdelend \DIFaddbegin \DIFadd{. Recently, \mbox{%DIFAUXCMD
\citet{clark2014}
}%DIFAUXCMD
presented an 
...
\DIFadd{We divide the methods for retrieving the magnetization direction }\DIFaddend into two
...
from their first-order magnetic moments. \DIFdelbegin \DIFdel{This author }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{phillips2005}
}%DIFAUXCMD
}\DIFaddend stresses 
that the method
...
the shape of the magnetic sources. \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{This assumptions about the underlying 
sources is able to reduce the nonuniqueness of the problem to a point that 
regularization or constraints are not required. Our method }\DIFaddend can be applied 
for \DIFdelbegin \DIFdel{interpreting
multiple sourceswith different magnetization directions. It does not require
the }\DIFdelend \DIFaddbegin \DIFadd{estimating the average magnetization direction of
multiple sources. It neither requires that all sources have the same 
magnetization direction nor the }\DIFaddend use of regularly spaced data on a horizontal
grid\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{. Besides, our method also }\DIFaddend contains
...
In general, the total-field anomaly is produced by a \DIFdelbegin \DIFdel{magnetized
susceptibility distribution }\DIFdelend \DIFaddbegin \DIFadd{distribution of 
magnetization }\DIFaddend which is anomalous with respect to the mean \DIFdelbegin \DIFdel{susceptibility }\DIFdelend \DIFaddbegin \DIFadd{induced magnetization
}\DIFaddend of the crust. Mathematically, $\Delta T^\mathrm{o}_{i}$ can be
written as
\begin{align}
 &
...
\end{align}
Different from Eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:goal-function-L2}), the parameter vector
$\vec{h}$ minimizing the Eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:goal-function-L1}) cannot be
...
system given by Eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:iteration-L1}) for obtaining the estimate
...
= 1, \ldots, L$ (Eq.~\ref{eq:mag-vector-j}). \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{However, }\DIFaddend magnetization vectors
are \DIFdelbegin \DIFdel{represented in Cartesian coordinates, however they are }\DIFdelend commonly represented
...
%DIFDELCMD < %%%
\DIFdelend Note that, according to Eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:hj}), the declinations $D_{j}$
...
estimate $\hat{\vec{h}}$ or the robust estimate $\tilde{\vec{h}}$ \DIFaddbegin \DIFadd{in Cartesian
coordinates}\DIFaddend , we
...
\DIFaddbegin \subsection{\DIFadd{Software implementation}}

\DIFadd{The inversion method described above
is implemented in version 0.3 of
the open-source Python language library
Fatiando a Terra}\footnote{\DIFadd{
http://www.fatiando.org/}}\footnote{\DIFadd{http://dx.doi.org/10.5281/zenodo.16205}}\DIFadd{.
We used }\emph{\DIFadd{matplotlib}} \DIFadd{\mbox{%DIFAUXCMD
\citep{Hunter2007}
}%DIFAUXCMD
for graphs and maps
and }\emph{\DIFadd{Mayavi}} \DIFadd{\mbox{%DIFAUXCMD
\citep{Ramachandran2011}
}%DIFAUXCMD
for 3D figures.
...
available as online supplementary material}\footnote{\DIFadd{
https://github.com/pinga-lab/Total-magnetization-of-spherical-bodies}}\footnote{\DIFadd{
http://dx.doi.org/10.5281/zenodo.16191}}\DIFadd{.
}
...
(Fig.~\ref{fig:synt1-data}b). \DIFaddbegin \DIFadd{The interfering anomalies reach
approximately $33 \%$ of the positive amplitude of the original
total-field anomaly shown in Fig.~\ref{fig:synt1-data}a.
}\DIFaddend Notice that this test violates the
...
\DIFaddbegin \subsection{\DIFadd{Complex tests}}

\DIFadd{In this section, we present the performance of our method in recovering
the magnetization direction of synthetic sources simulating complex
geological scenarios.
}

\subsubsection{\DIFadd{Strong-interfering anomalies}}

\DIFadd{Figure \ref{fig:overlapping-prisms} shows two synthetic sources
simulating uniformly magnetized bodies embedded in nonmagnetic
host rocks. Both sources are rectangular prisms with
horizontal dimensions equal to 80}\,\unit{m} \DIFadd{and 20}\,\unit{m} \DIFadd{along
the $x$ and $y$ directions, respectively, vertical dimension
equal to 70}\,\unit{m} \DIFadd{and depth of the top at $z = 10$}\,\unit{m}
\DIFadd{(red prisms in Fig. \ref{fig:overlapping-prisms}).
One source has its centre at $xc = 0$}\,\unit{m}\DIFadd{, $yc = -30$}\,\unit{m}
\DIFadd{and $zc = 45$}\,\unit{m} \DIFadd{while the other one has its centre shifted
60}\,\unit{m} \DIFadd{in the positive $y$ direction (pinpointed black dots
in Fig. \ref{fig:overlapping-prisms}). Both sources
have a Koenigsberger ratio equal to $3$ and the same induced
magnetization with intensity $3$}\,\unit{A\,m^{-1}}\DIFadd{, declination
$0$}{\degree} \DIFadd{and inclination $-30$}{\degree}\DIFadd{. The remanent
magnetization of the sources has the same inclination equal to
$0$}{\degree}\DIFadd{, but different declinations equal to $30$}{\degree}
\DIFadd{(eastern source) and $-30$}{\degree} \DIFadd{(western source). The total field
...
the plane with constant vertical coordinate $z=-10$}\,\unit{m}\DIFadd{, extending
from -200 to 200}\,\unit{m} \DIFadd{in both $x$ and $y$ directions. The
...
\DIFadd{By assigning the correct positions of the centres of the simulated bodies, we
...
\subsubsection{\DIFadd{Igneous intrusion within a sedimentary basin}}

\DIFadd{Figure \ref{fig:intrusion-profile} shows a 2D schematic 
...
which is magnetized by induction (with intensity $0.1$}\,\unit{A\,m^{-1}}\DIFadd{)
...
field has inclination $-39.8$}{\degree} \DIFadd{and declination $-22.5$}{\degree}\DIFadd{. 
The synthetic intrusion has a reversed magnetization with inclination 
$I = 39.8$}{\degree}\DIFadd{, declination $D = 157.5$}{\degree} \DIFadd{and intensity
$6$}\,\unit{A\,m^{-1}}\DIFadd{. In this example, the total-field anomaly produced
...
0 to 10}\,\unit{km} \DIFadd{and from 45 to 55}\,\unit{km} \DIFadd{in $x$ and $y$ directions,
respectively, on a plane with constant vertical coordinate -800}\,\unit{m}\DIFadd{. 
...
equal to 2}\,\DIFadd{nT.
}

\DIFadd{We applied our method to the total-field anomaly shown in 
...
\DIFaddend \section{Application to field data}
...
\DIFadd{By using the }\DIFaddend estimated magnetization directions \DIFdelbegin \DIFdel{for reducing another }\DIFdelend \DIFaddbegin \DIFadd{obtained from the 
simple dipolar }\DIFaddend total-field \DIFaddbegin \DIFadd{anomaly shown in Fig. \ref{fig:TFA-Diorama}, 
we reduced to the pole another complex non-dipolar total-field }\DIFaddend anomaly
(Fig. \DIFdelbegin \DIFdel{~\ref{fig:TFA-MCG})to the pole}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:TFA-MCG})}\DIFaddend . This total-field anomaly is
...
geological time interval. \DIFaddbegin \DIFadd{Notice that in this field-data application, the 
...
required. Our method can be applied for \DIFdelbegin \DIFdel{interpreting }\DIFdelend \DIFaddbegin \DIFadd{determining the average magnetization
direction within }\DIFaddend multiple sources with different magnetization directions. 
...
\begin{acknowledgements}
  ...
  research.  V.~C.~F.~Barbosa was supported by a~fellowship from \DIFdelbegin \DIFdel{CNPQ
  }\DIFdelend \DIFaddbegin \DIFadd{CNPq
  }\DIFaddend (Conselho Nacional de Desenvolvimento e Tecnol\'{o}gico) and
  ...
  Estado do Rio de Janeiro) under contracts \DIFdelbegin \DIFdel{: }\DIFdelend E-26/103.175/2011 and
  E-26/111.152/\DIFdelbegin \DIFdel{2014.
}\DIFdelend \DIFaddbegin \DIFadd{2014 and CNPq under contract 445752/2014-9.
...
uncertainties $\sigma_\mathrm{D}$ and $\sigma_\mathrm{I}$ \DIFaddbeginFL \DIFaddFL{(shown between
parenthesis) }\DIFaddendFL are calculated by
...
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{crr}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{crcrc}
\DIFaddendFL \tophline
&Sphere & \DIFdelbeginFL \DIFdelFL{Prism }\DIFdelendFL \DIFaddbeginFL &\DIFaddFL{Prism }& \DIFaddendFL \\
\middlehline
$D$ &$-10.00000$ & \DIFaddbeginFL \DIFaddFL{-- }&\DIFaddendFL $-40.00000$ \DIFaddbeginFL & \DIFaddFL{-- }\DIFaddendFL \\
$\hat{D}$ &$-10.07141$ &\DIFaddbeginFL \DIFaddFL{($0.00001$) }&\DIFaddendFL $-40.63733$ \DIFdelbeginFL %DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{D}$ }\DIFdelendFL &\DIFdelbeginFL \DIFdelFL{$0.00000$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00113$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
$\tilde{D}$ &$-10.03229$ &\DIFdelbeginFL \DIFdelFL{$-40.24585$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\tilde{\sigma}_\mathrm{D}$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00130$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL &\DIFaddbeginFL \DIFaddFL{$-40.24585$ }&\DIFaddFL{(}\DIFaddendFL $0.03601$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
...
$\hat{I}$ &$-19.99437$ &\DIFaddbeginFL \DIFaddFL{($0.00001$) }&\DIFaddendFL $31.04075$ \DIFdelbeginFL %DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{I}$ }\DIFdelendFL &\DIFdelbeginFL \DIFdelFL{$0.00000$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00068$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
$\tilde{I}$ &$-20.01263$ &\DIFdelbeginFL \DIFdelFL{$30.60551$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\tilde{\sigma}_\mathrm{I}$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00042$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL &\DIFaddbeginFL \DIFaddFL{$30.60551$ }&\DIFaddFL{(}\DIFaddendFL $0.02047$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
\bottomhline
\end{tabular}
...
\middlehline
$D$ &$-10.00000$ & \DIFaddbeginFL \DIFaddFL{-- }&\DIFaddendFL $-40.00000$ \DIFaddbeginFL & \DIFaddFL{-- }\DIFaddendFL \\
$\hat{D}$ &$-4.28547$ &\DIFaddbeginFL \DIFaddFL{($0.00001$) }&\DIFaddendFL $-23.63607$  \DIFdelbeginFL %DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{D}$ }\DIFdelendFL &\DIFdelbeginFL \DIFdelFL{$0.00000$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00130$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
$\tilde{D}$ &$-8.73648$ &\DIFdelbeginFL \DIFdelFL{$-39.37397$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\tilde{\sigma}_\mathrm{D}$ }%DIFDELCMD < &%%%
...
\DIFaddbeginFL \middlehline
\DIFaddendFL $I$ &$-20.00000$ & \DIFaddbeginFL \DIFaddFL{-- }&\DIFaddendFL $30.00000$ \DIFaddbeginFL & \DIFaddFL{-- }\DIFaddendFL \\
$\hat{I}$ &$-25.11757$ &\DIFaddbeginFL \DIFaddFL{($0.00001$) }&\DIFaddendFL $39.08012$  \DIFdelbeginFL %DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\hat{\sigma}_\mathrm{I}$ }\DIFdelendFL &\DIFdelbeginFL \DIFdelFL{$0.00000$ }%DIFDELCMD < &%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL $0.00064$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL \\
$\tilde{I}$ &$-21.75674$ &\DIFdelbeginFL \DIFdelFL{$33.40926$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$\tilde{\sigma}_\mathrm{I}$ }%DIFDELCMD < &%%%
...
\begin{table}[t]
\caption{\DIFaddFL{Test with synthetic data (Fig. \ref{fig:overlapping-prisms-data})
...
using Eqs. (\ref{eq:sigmaDj}) and (\ref{eq:sigmaIj}). All values are in
degree (}{\degree}\DIFaddFL{). We use a~caret ($^{\wedge}$) and a~tilde ($^{\sim}$) to
distinguish the quantities computed by using, respectively, the least-squares
$\hat{\vec{h}}$ and robust $\tilde{\vec{h}}$ estimates.}}
\begin{tabular}{crcrc}
\tophline
&\DIFaddFL{Eastern prism }& & \DIFaddFL{Western prism }& \\
\middlehline
\DIFaddFL{$D$ }&\DIFaddFL{$-23.41322$ }& \DIFaddFL{-- }&\DIFaddFL{$23.41322$ }& \DIFaddFL{-- }\\
\DIFaddFL{$\hat{D}$ }&\DIFaddFL{$-31.45370$ }&\DIFaddFL{($0.00008$) }&\DIFaddFL{$30.67233$ }&\DIFaddFL{($0.00875$) }\\
\DIFaddFL{$\tilde{D}$ }&\DIFaddFL{$-26.57707$ }&\DIFaddFL{($0.01778$) }&\DIFaddFL{$25.25037$ }&\DIFaddFL{($0.13333$) }\\
\middlehline
\DIFaddFL{$I$ }&\DIFaddFL{$-7.54509$ }& \DIFaddFL{-- }&\DIFaddFL{$-7.54509$ }& \DIFaddFL{-- }\\
\DIFaddFL{$\hat{I}$ }&\DIFaddFL{$-9.23914$ }&\DIFaddFL{($0.00004$) }&\DIFaddFL{$-9.06131$ }&\DIFaddFL{($0.00628$) }\\
\DIFaddFL{$\tilde{I}$ }&\DIFaddFL{$-7.98897$ }&\DIFaddFL{($0.00408$) }&\DIFaddFL{$-11.05456$ }&\DIFaddFL{($0.06386$) }\\
\bottomhline
\end{tabular}
...
\begin{table}[t]
\caption{\DIFaddFL{Test with synthetic data (Fig. \ref{fig:intrusion-data})
...
\begin{tabular}{crc}
\tophline
&\DIFaddFL{Synthetic intrusion }\\
\middlehline
\DIFaddFL{$D$ }&\DIFaddFL{$157.50000$ }& \DIFaddFL{--}\\
\DIFaddFL{$\hat{D}$ }&\DIFaddFL{$167.39501$ }&\DIFaddFL{($0.00060$) }\\
\DIFaddFL{$\tilde{D}$ }&\DIFaddFL{$164.19461$ }&\DIFaddFL{($0.02669$) }\\
\middlehline
\DIFaddFL{$I$ }&\DIFaddFL{$39.80000$ }& \DIFaddFL{-- }\\
\DIFaddFL{$\hat{I}$ }&\DIFaddFL{$37.33816$ }&\DIFaddFL{($0.00035$) }\\
\DIFaddFL{$\tilde{I}$ }&\DIFaddFL{$39.99470$ }&\DIFaddFL{($0.00939$) }\\
\bottomhline
\end{tabular}
...
\caption{\DIFdelbeginFL \DIFdelFL{Application to field data on the Goi}%DIFDELCMD < \'{a}%%%
\DIFdelFL{s Alkaline Province
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Strong-interfering anomalies. Synthetic prisms
}\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{GAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in red}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{with side lengths
equal to 80}\,\unit{m}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{Brazil. Simplified geological map of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{20}\,\unit{m} \DIFaddFL{and 70}\,\unit{m} \DIFaddFL{along
}\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{study area}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$x$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{which is
  represented by }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$y$ and $z$ directions, respectively. Both prisms have }\DIFaddendFL a \DIFdelbeginFL \DIFdelFL{~red dot in the inset map }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{depth
}\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{Brazil}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the top at $z = 10$}\,\unit{m}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{inset also
  shows }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{eastern prism has its centre at
$xc = 0$}\,\unit{m}\DIFaddFL{, $yc = -30$}\,\unit{m} \DIFaddFL{and $zc = 45$}\,\unit{m} \DIFaddFL{while
western prism has its centre shifted 60}\,\unit{m} \DIFaddFL{in }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{Goi}%DIFDELCMD < \'{a}%%%
\DIFdelFL{s }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{positive $y$
direction }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{dark grey area}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pinpointed black dots}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{and Minas Gerais (light grey
  area) states}\DIFdelendFL .\DIFdelbeginFL \DIFdelFL{The total-field anomaly over the area delimited by the
  ...
  Goi}%DIFDELCMD < \'{a}%%%
\DIFdelFL{s, 2 -- Diorama, 3 -- C}%DIFDELCMD < \'{o}%%%
\DIFdelFL{rrego dos Bois, 4 -- Morro do
  Macaco and 5 -- Fazenda Buriti.}\DIFdelendFL }
\DIFdelbeginFL %DIFDELCMD < \label{fig:geology-study-area}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:overlapping-prisms}
\DIFaddendFL \end{figure*}


\begin{figure*}[t]
\includegraphics[width=120mm]{Figures/npgd-2014-0069-f09}
\DIFaddbeginFL \caption{\DIFaddFL{Strong-interfering anomalies. Noise-corrupted
...
\caption{\DIFaddFL{Igneous intrusion. 2D schematic representation of a
...
\caption{\DIFaddFL{Igneous intrusion. Noise-corrupted total-field anomaly
...
\caption{\DIFaddFL{Igneous intrusion. 3D view of the intrusion (red prisms)
...
\begin{figure}[t]
  ...
  \caption{\DIFaddFL{Igneous intrusion. The upper panel shows the true
...
\caption{\DIFaddFL{Application to field data on the Goi}\'{a}\DIFaddFL{s Alkaline Province
  ...
  shows the Goi}\'{a}\DIFaddFL{s (dark grey area) and Minas Gerais (light grey
  area) states. The total-field anomaly over the area delimited by the
  red rectangle is shown in Fig.~\ref{fig:TFA-Diorama}. The
  coordinates are referred to the WGS84 datum. The numbers indicate
  the main alkaline complexes in this region: 1 -- Montes Claros de
  Goi}\'{a}\DIFaddFL{s, 2 -- Diorama, 3 -- C}\'{o}\DIFaddFL{rrego dos Bois, 4 -- Morro do
...
\begin{figure*}[t]
  ...
  \DIFaddendFL \caption{Application to field data on the Goi\'{a}s Alkaline Province (GAP),
...
\DIFaddend \begin{figure}[t]
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=70mm]{Figures/npgd-2014-0069-f10}
%DIFDELCMD <   %%%
...
\DIFaddend \begin{figure*}[t]
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=120mm]{Figures/npgd-2014-0069-f11}
%DIFDELCMD < %%%
...
\DIFaddend \begin{figure}[t]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=80mm]{Figures/npgd-2014-0069-f12}
%DIFDELCMD < %%%
