...
%DIF ADD Total_magnetization_estimation_revised.tex   Tue Mar 17 20:36:47 2015
...
%DIFDELCMD < %%%
\DIFdelend Note that, according to Eq. (\ref{eq:hj}), the declinations $D_{j}$
...
(\ref{eq:sensibility-matrix})\DIFaddbegin \DIFadd{.
}

\subsection{\DIFadd{Complex tests}}
...
\DIFadd{Figure XXX shows the synthetic noise-corrupted total-field
anomaly produced by two uniformly magnetized bodies embedded in 
nonmagnetic host rocks. Both sources are rectangular prisms with 
...
equal to 70}\,\unit{m} \DIFadd{and depth of the top at $z = 10$}\,\unit{m}\DIFadd{.
...
60}\,\unit{m} \DIFadd{in the positive $y$ direction (Fig. XXX). Both sources
...
anomaly produced by these synthetic bodies was calculated at $N = 2\,601$
